        ...
        if form.is_valid():
            ...
            if request.user.email:
                send_mail(
                    ...
                    message=f"Your appointment on {appt.date} for reason '{appt.reason}' has been submitted and is pending confirmation.",
                    from_email=None,
                    recipient_list=[request.user.email],
                    fail_silently=True,
                )

            messages.success(request, "Appointment submitted and pending confirmation.")
            return redirect("home")
    else:
        form = AppointmentForm(specialty=preselected_specialty_id)

    specialties = Specialty.objects.all().order_by("name")
    return render(request, "appointments/patient_book.html", {"form": form, "specialties": specialties})
...
def admin_confirm(request, pk):
    ...
    if appt.patient.email:
        send_mail(
            ...
            message=f"Your appointment on {appt.date} for reason '{appt.reason}' has been confirmed.",
            from_email=None,
...
def admin_cancel(request, pk):
    ...
    if appt.patient.email:
        send_mail(
            ...
            message=f"Your appointment on {appt.date} for reason '{appt.reason}' has been cancelled.",
            from_email=None,
            recipient_list=[appt.patient.email],
            fail_silently=True,
        )
...
def admin_dashboard(request):
    pending = Appointment.objects.filter(status=Appointment.Status.PENDING).order_by("date")
    confirmed = Appointment.objects.filter(status=Appointment.Status.CONFIRMED).order_by("date")
    cancelled = Appointment.objects.filter(status=Appointment.Status.CANCELLED).order_by("-date")

    doctors = Doctor.objects.select_related("user", "specialty").all().order_by("user__first_name", "user__last_name")
